...
export const JournalAI: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (content.trim().length < 10) {
      setError('Journal content must be at least 10 characters.');
      return;
    }

    if (content.trim().length > 2000) {
      setError('Journal content must not exceed 2000 characters.');
      return;
    }

    setIsSubmitting(true);
    try {
      await addJournalAiEntry(content.trim());
      setContent('');
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="pb-4 border-b border-slate-200 dark:border-[#334155] flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-2xl font-display font-extrabold text-neutral-slate dark:text-[#F8FAFC]">AI-Powered Journal</h1>
          <p className="text-xs text-neutral-outline dark:text-[#CBD5E1]">Write daily reflections. Groq AI analyzes your sentiment to gauge burnout risks.</p>
        </div>
      </div>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="flex justify-between items-center">
                  <span className={`text-[10px] ${content.length > 2000 ? 'text-error' : 'text-neutral-outline dark:text-[#CBD5E1]'}`}>
              ...
              <div className="flex justify-between items-center pt-2">
                <span className="text-[10px] text-neutral-outline dark:text-[#CBD5E1] flex items-center">
                  ...
                  AI sentiment analysis powered by Groq
                </span>
                
                <button
                  type="submit"
                  disabled={isSubmitting || content.trim().length < 10 || content.trim().length > 2000}
                  className="bg-primary dark:bg-[#4F46E5] text-white font-semibold px-5 py-2 rounded-lg text-xs hover:bg-primary/95 dark:hover:bg-[#4338CA] transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
                  {isSubmitting ? (
                    <>
          ...
          <div className="bg-primary/5 dark:bg-[#4F46E5]/10 border border-primary/10 dark:border-[#4F46E5]/20 rounded-xl p-4 flex items-start space-x-3 text-primary dark:text-[#A5B4FC]">
            ...
            <div className="text-xs">
              ...
              <span className="block text-[11px] text-primary/85 dark:text-[#A5B4FC]/85 leading-relaxed mt-0.5">
                Administrators can only view aggregate sentiment totals (e.g. "Mostly Positive" or "Balanced") and never your actual text entries. Your secrets are safe.
              </span>
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {journalAiEntries.map((entry) => (
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${getSentimentBadge(entry.sentiment)} flex items-center`}>
                      <span className="mr-1">{getSentimentIcon(entry.sentiment)}</span>
                  ...
                  <div className="flex justify-end pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={() => deleteJournalAiEntry(entry.id)}
